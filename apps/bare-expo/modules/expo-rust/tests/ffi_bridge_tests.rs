...
//! compiled in EXPO_RUST_STANDALONE mode. Unlike unit tests that only test Rust logic,
//! these call through the real FFI boundary to the C++ jsi_shim functions.

use expo_rust::prelude::*;
...
fn callback_registry_roundtrip() {
    ...
    let id = expo_rust::bridge::register_callback(|_rt, args| {
...
fn callback_registry_unique_ids() {
    let id1 = expo_rust::bridge::register_callback(|_rt, _| JsValue::Undefined);
    let id2 = expo_rust::bridge::register_callback(|_rt, _| JsValue::Undefined);
    let id3 = expo_rust::bridge::register_callback(|_rt, _| JsValue::Undefined);
    assert_ne!(id1, id2);
    assert_ne!(id2, id3);
    assert_ne!(id1, id3);
}
...
fn make_test_runtime() -> Runtime {
    Runtime {
        handle: expo_rust::bridge::ffi::RuntimeHandle {
